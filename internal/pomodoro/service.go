...
func (s *Service) Start(ctx context.Context, plannedMinutes int, linkedTodoID string) (*Session, error) {
	now := time.Now()
	session := &Session{
		ID:             idgen.New(),
		StartedAt:      now,
...
func (s *Service) GetTodayMinutes(ctx context.Context, timezone string) (int, error) {
	return s.repo.GetTodayMinutes(ctx, timezone)
}
